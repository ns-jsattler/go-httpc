...
func (r *Request) DoAndGetReader(ctx context.Context) (*http.Response, error) {
	...
	err := r.backoff.BackoffCtx(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return httpcerrors.NewClientErr("encode body", err, nil, r.metaErrOpts()...)
		}

		req, err := http.NewRequest(r.method, r.addr, body)
		if err != nil {
			return httpcerrors.NewClientErr("new req", err, nil, r.metaErrOpts()...)
		}
...
func (r *Request) do(ctx context.Context) error {
	...
	if err := r.decodeFn(resp.Body); err != nil {
		var opts []httpcerrors.ClientOptFn
		if isRetryErr(err) {
			opts = append(opts, httpcerrors.Retry())
		}
		opts = append(opts, r.metaErrOpts()...)
		return httpcerrors.NewClientErr("decode", err, resp, opts...)
	}
	return nil
}
...
func (r *Request) metaErrOpts() []httpcerrors.ClientOptFn {
	if len(r.logMeta) == 0 {
		return nil
	}
	var pairs []string
	...
	return []httpcerrors.ClientOptFn{httpcerrors.Meta(pairs[0], pairs[1], pairs[2:]...)}
}

func (r *Request) statusErrOpts(status int) []httpcerrors.ClientOptFn {
	var opts []httpcerrors.ClientOptFn
	if statusMatches(status, r.retryStatusFns) {
		opts = append(opts, httpcerrors.Retry())
	}
	if statusMatches(status, r.notFoundFns) {
		opts = append(opts, httpcerrors.NotFound())
	}
	if statusMatches(status, r.existsFns) {
		opts = append(opts, httpcerrors.Exists())
	}
	opts = append(opts, r.metaErrOpts()...)
	return opts
}
...
func (r *Request) responseErr(resp *http.Response, err error) error {
	...
	opts = append(opts, r.metaErrOpts()...)
	return httpcerrors.NewClientErr("do", err, resp, opts...)
}
